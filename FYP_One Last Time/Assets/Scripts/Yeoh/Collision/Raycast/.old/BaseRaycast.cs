...
public class BaseRaycast : SlowUpdate
{
    ...
    public virtual bool RayHit(out GameObject target)
    {
        ...
        {
            if(IsHitValid(rayHit, reverseDoubleCheck, out var hitObj))
            {
                target = hitObj;
                return true;
            }
        ...
        {
            if(ReverseRayHit(GetRayDir(), out var reverse_hit_obj))
            {
                target=reverse_hit_obj;
                return true;
            }
        }
    ...
    protected bool ReverseRayHit(Vector3 forward_dir, out GameObject reverse_hit_obj)
    {   
        Vector3 end_pos = GetEndPos(origin.position, forward_dir);

        if(Physics.Raycast(end_pos, -forward_dir, out var reverseHit, range, targetLayers, QueryTriggerInteraction.Ignore))
        {
            return IsHitValid(reverseHit, false, out reverse_hit_obj);
        }
        reverse_hit_obj=null;
        return false;
    }
    ...
    protected bool IsHitValid(RaycastHit hit, bool reverse_check, out GameObject hitObj)
    {
